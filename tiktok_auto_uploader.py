...
class TikTokAutoUploader:
    ...
    def _click_element(self, selectors: list, timeout: int = 10000) -> bool:
        ...
        if found_selector:
            try:
                ...
                print(f"[WARN]  Could not click {found_selector}: {e}")
                return False
        return False
    ...
    def _wait_for_login(self, page: Page, timeout: int = 300000):
        ...
        """
        print("\n" + "="*70)
        print("[LOCK] AUTHENTIFICATION GOOGLE REQUISE")
        print("="*70)
        ...
        try:
            ...
            print(f"[ERROR] Timeout ou erreur d'authentification: {e}")
            return False
    ...
    def login_with_google(self):
        ...
        for channel in ['chrome', 'msedge', 'chromium', None]:
            try:
                ...
                print(f"[OK] Navigateur lancé avec succès!")
                break
            except Exception as e:
                print(f"[WARN]  Échec avec {channel or 'default'}: {e}")
                if channel is None:
        ...
        if self.session_file.exists():
            print("[FOLDER] Chargement de la session existante...")
            try:
                with open(self.session_file) as f:
                    storage_state = json.load(f)
                context_options['storage_state'] = storage_state
            except Exception as e:
                print(f"[WARN]  Impossible de charger la session: {e}")
        ...
        if self._is_logged_in():
            print("[OK] Déjà connecté!")
            return True

        # Need to login
        print("[KEY] Tentative de connexion avec Google...")
        ...
        if not self._click_element(login_selectors, timeout=10000):
            ...
            print("[WARN]  Bouton de connexion introuvable, tentative de navigation directe...")
        ...
        if not self._click_element(google_selectors, timeout=10000):
            ...
            print("[ERROR] Impossible de trouver le bouton Google")
            return False
        ...
        try:
            ...
            print(f"[WARN]  Impossible de sauvegarder la session: {e}")
    ...
    def upload_video(
        ...
        """
        if not self.page:
            print("[ERROR] Pas de session active. Appelez login_with_google() d'abord.")
            return False

        try:
            print(f"\n{'='*70}")
            print(f"[UPLOAD] UPLOAD TIKTOK: {video_path.name}")
            print(f"{'='*70}\n")
            ...
            if not self._is_logged_in():
                print("[ERROR] Session expirée. Reconnectez-vous.")
                return False

            # Upload video file
            print("[FOLDER] Upload du fichier vidéo...")
            ...
            while time.time() - start_time < processing_timeout:
                ...
                print("[WARN]  Timeout lors du traitement, continuant quand même...")

            self._save_debug_screenshot("13_video_processed")

            # Add caption (title + hashtags + description)
            print("[NOTE] Ajout du titre et hashtags...")
            caption = f"{title}\n\n"
            ...
            for selector in caption_selectors:
                try:
                    ...
                    if caption_elem.is_visible(timeout=3000):
                        ...
                        self._human_delay(1, 2)
                        print(f"[OK] Caption ajoutée ({len(caption)} caractères)")
                        caption_added = True
                        break
                except Exception as e:
                    continue
            ...
            if not caption_added:
                ...
                print("[WARN]  Impossible d'ajouter la caption")
            ...
            if publish:
                print("[START] Publication de la vidéo...")
                publish_selectors = [
...
def main():
    ...
    # Login with Google
    print("\n[KEY] Connexion à TikTok avec Google...")
    if not uploader.login_with_google():
        print("[ERROR] Échec de la connexion")
        return

    print("\n[OK] Connexion réussie!")
    print("[NOTE] Le système est maintenant prêt à uploader des vidéos.")
